...
public class CciStrategy extends TradeStrategy {
	...
	public void execute(StockStatus stockStatus, Account account) {
		...
		if ((targetHolding != null && !stockStatus.isStrongest() && stockStatus.dropsTopDvg())
				&& !stockStatus.isSuperHighOpen() && stockStatus.isSellingRange()) {
			int qty = 0;
		...
		for (Order order : orders) {
			if (order.getTxType() == TransactionType.SELL && order.getStatus() == OrderStatus.OPEN
				...
				if (curFactor > Constants.STOP_ORDER_LOCKWIN_FACTOR) {
					double newStop = Util.trim(
							order.getStopPrice() * (1.0 + (curFactor - Constants.STOP_ORDER_TRAILING_FACTOR) / 2));
					if (newStop > order.getStopPrice()) {
						order.setStopPrice(newStop);
						logger.info("[" + Util.format(stockStatus.getTimestamp())
								+ "] Raising Stop price to lock profit on " + order);
					}
				}
				...
				// set trailing stop price
					...
					double newStop = Util
							.trim(order.getStopPrice() * (1.0 + curFactor - Constants.STOP_ORDER_TRAILING_FACTOR));
					if (newStop > order.getStopPrice()) {
						order.setStopPrice(newStop);
						logger.info("[" + Util.format(stockStatus.getTimestamp()) + "] Moved Trailing Stop price up on "
								+ order);
					}
				}
			}
